...
- This repository is a Windows-first local scanning toolkit, not a packaged application. The main workflows depend on bundled Windows executables (`ts.exe`, `spray.exe`, `ehole.exe`), `.bat` wrappers, and Windows shell behavior used in `1.py` (`start cmd /c`, `ping -n`, `chcp 65001`).
...
- Full workflows also require the repo-local binaries `ts.exe`, `spray.exe`, and `ehole.exe`.
...
### Run the full spray → filter → ehole pipeline
...
- Runs `ehole finger -l <filtered.txt> -o <dated.xlsx> -t 10`
- Beautifies the ehole workbook and archives outputs into a `MMDD/` folder
...
- `.xlsx`/`.xls` input: treat as ehole output and beautify in place or after copying to a new workbook
...
- Use the smallest relevant script as a manual verification step:
  ...
  - `python 1.py` for the full spray → filter → ehole pipeline
...
- `1.py` is the top-level Windows orchestration layer for the spray → process_data → ehole flow. It runs external commands, streams their output to timestamped log files, monitors process completion and progress files, creates a date folder, filters 200-status URLs, invokes ehole, and moves finished artifacts into `MMDD/` output directories.
- `process_data.py` is the shared normalization/reporting layer used by `1.py`. For spray JSONL it preserves the original rows in Excel, strips `redirect_url`, flattens selected nested fingerprint fields, emits a sibling URL list, and leaves the 200-only filtering decision to `1.py`. For ehole workbooks it performs in-place beautification and adds extra summary sheets when possible.
...
- Root-level intermediate outputs include `port.txt`, `url.txt`, `res.json`, `res_processed.xlsx`, and `res_processed.txt`. Finalized spray and ehole artifacts are archived into date folders named `MMDD/`.
...
- `process_data.py` is the canonical place for output shaping and workbook beautification. If a change affects spray/ehole Excel structure or URL extraction, update shared logic there rather than forking formatting logic in `1.py`.
